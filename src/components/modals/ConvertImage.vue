<template>
  <div id="image-loader">
    <h1 class="image-loader--header">
      <IconImageAdd class="image-loader--header-icon"/>
      Convert an Image Into a Pattern
    </h1><!-- header -->
    ...
    <div class="cropper--container" v-show="isCropping">
      ...
      <div class="cropper--mural-inputs" v-show="patternType == 9">
        <div class="cropper--mural-inputs-columns">
          <label>Patterns Wide</label><br/>
          <input type="range" min="1" max="8" v-model="muralWide"/>
          <input type="number" min="1" max="8" v-model="muralWide"/>
        </div>
        <div class="cropper--mural-inputs-rows">
          <label>Patterns Tall</label><br/>
          <input type="range" min="1" max="8" v-model="muralTall"/>
          <input type="number" min="1" max="8" v-model="muralTall"/>
        </div>
      </div>

      <div id="cropper--buttons">
        <button @click="tryAgain">Upload an Image File</button>
    ...
    <div class="preview--container" v-show="!isCropping">
      ...
      <div class="preview--options">
        <ul class="preview--options-option">
        ...
        <ul class="preview--options-option" v-show="muralTall > 1 || muralWide > 1">
      ...
      <div id="preview--buttons">
        ...
        <button @click="convert(outputs)">Convert!</button>
      </div>
    ...
    onCrop({coordinates, canvas}) {
      ...
      for (let x = 0; x < this.muralWide; x++) {
        for (let y = 0; y < this.muralTall; y++) {
          ...
          //Create separate palettes for each pattern
          if (!this.convert_samepal) {
            switch (this.convert_method) {
              case 'quantize': this.image_quantize(imgdata); break;
              case 'rgb': this.image_rgb(imgdata); break;
...
.cropper--container {
  ...
  .cropper--mural-inputs {
    ...
    .cropper--mural-inputs-column, 
    .cropper--mural-inputs-row {
      flex: 50%;
      flex-direction: column;
