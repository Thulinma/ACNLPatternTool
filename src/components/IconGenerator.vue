...
@Component
export default class IconGenerator extends Vue {
  $refs!: { iCanvas: HTMLCanvasElement; }
  
  @Prop({
    required: true,
  }) pattern!: ConstructorParameters<typeof DrawingTool>[0];
  ...
  @Watch('pattern')
  onPatternChanged(newData: typeof this.pattern) {
    this.draw(newData);
  }
  ...
  async draw(newData: typeof this.pattern) {
    ...
    let ctx = this.$refs.iCanvas.getContext("2d") as CanvasRenderingContext2D;
    ...
    if (this.decoration) {
      ...
      ctx.fillStyle = ctx.createPattern(bgCanvas, "repeat") as CanvasPattern;
    ...
    if (this.text) {
      ...
      const txtBg = ctx.createPattern(bgCanvas, "repeat") as CanvasPattern;

      const drawTxtWithBg = (x: number, y: number, txt: string, fore: string) => {
